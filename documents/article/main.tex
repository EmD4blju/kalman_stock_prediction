...
W tym celu zaprojektowano i zaimplementowano trzy odrębne modele, z których każdy służył weryfikacji innej hipotezy badawczej:
\begin{itemize}
...
\end{itemize}
W niniejszym rozdziale przedstawiono szczegółowy opis metodologii badawczej zastosowanej w pracy, obejmujący proces przygotowania danych, implementację modeli, procedurę uczenia oraz metryki oceny wydajności.
...
Proces ten obejmował następujące kroki odpowiednio: pobrania danych, przetwarzania wstępnego, inżynierii cech oraz podziału na zbiory treningowy, walidacyjny i testowy \cite{TODO_preprocessing_practices}.
...
\vspace{0.5cm}
Tabela \ref{tab:model_architecture_params} przedstawia zestawienie kluczowych hiperparametrów architektury każdego z trzech modeli LSTM użytych w eksperymentach.
\begin{table}[H]
    \centering
...
% Proces uczenia każdego z trzech modeli został przeprowadzony w sposób niezależny, z wykorzystaniem tych samych zasad i procedur, aby zapewnić rzetelność i porównywalność wyników.
% Obejmował on strojenie hiperparametrów, definiowanie funkcji straty oraz wybór algorytmu optymalizacji, własciwy trening na danych treningowych oraz monitorowanie wydajności na zbiorze walidacyjnym.
...
\end{itemize}
Dla każdego modelu przeprowadzono 50 prób optymalizacji (ang. trials), a najlepsze zestawy hiperparametrów zostały następnie w ostatecznym treningu modeli.

\subsubsection{Przebieg procesu uczenia}
Finalny trening modeli przeprowadzono na zbiorze treningowym, wykorzystując zoptymalizowane zestawy hiperparametrów. 
...
Wydajność modelu w trakcie uczenia była na bieżąco monitorowana na zbiorze walidacyjnym. 
...
Do oceny wydajności modeli w trakcie procesu uczenia zastosowano trzy metryki:
\begin{itemize}
    ...
    \end{equation}
    gdzie \( y_i \) to rzeczywista wartość, \( \hat{y}_i \) to wartość przewidywana przez model, a \( n \) to liczba próbek.
    \item \textbf{R-squared (R²):} Miara dopasowania modelu, wskazująca, jaka część wariancji danych jest wyjaśniona przez model.
    \begin{equation}
...
Dzięki modularnej naturze Kedro, każdy etap -- od strojenia hiperparametrów za pomocą Optuny, po finalny trening modeli -- został zdefiniowany jako oddzielny, reużywalny węzeł (ang. \textit{node}). 
Parametry procesu, takie jak przeszukiwane zakresy hiperparametrów czy liczba prób optymalizacji, zostały odseparowane od logiki kodu i umieszczone w plikach konfiguracyjnych. 
