...
Formalnie, w sieciach RNN, stan ukryty \( \mathbf{h}_t \) w czasie \( t \) jest obliczany na podstawie bieżącego wejścia \( \mathbf{x}_t \) oraz poprzedniego stanu ukrytego \( \mathbf{h}_{t-1} \) \cite{TODO_RNN_cells}:
\begin{equation}
...
W celu wygenerowania prognozy \( \mathbf{\hat{y}}_t \) w czasie \( t \), sieć wykorzystuje stan ukryty \cite{TODO_RNN_output}:
\begin{equation}
...
Tak jak w przypadku klasycznej propagacji wstecznej, BPTT polega na obliczaniu gradientów funkcji kosztu względem wag i obciążeń sieci \cite{TODO_BPTT}.
