...
W niniejszym rozdziale przygotowano zestawienie wyników przeprowadzonych eksperymentów wedle opisanej wcześniej metodyki.
W rozdziale przedstawiono wyniki poszczególnych modeli, ich porównanie oraz analizę uzyskanych rezultatów.
...
Jego celem było ustalenie punktu odniesienia, jakie wyniki jest w stanie osiągnąć stosunkowo prosta architektura LSTM w kontekście predykcji cen giełdowych.
Tabela \ref{tab:base_model_metrics} przedstawia osiągnięte przez model bazowy metryki: %! CHECK IT
...
Uzyskany błąd RMSE na poziomie 4.5626 oznacza, że średnie odchylenie prognoz od rzeczywistych cen zamknięcia wynosi około 4.56 jednostek (dolarów).
Wskaźnik \(R^2\) na poziomie 0.8730 wskazuje, że model wyjaśnia około 87.3\% wariancji w danych testowych, co sugeruje dobrą zdolność do generalizacji.

Rysunki \ref{fig:base_model_predictions} oraz \ref{fig:base_model_error_distribution} ilustrują porównanie przewidywanych cen z rzeczywistymi wartościami oraz rozkład błędów predykcji.
Na wykresie porównawczym widać, że model bazowy dobrze odwzorowuje ogólny trend cen, choć występują pewne odchylenia w szczegółach.
Histogram błędów pokazuje, że większość predykcji mieści się w niewielkim zakresie od rzeczywistych wartości.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{assets/base_actual_vs_predicted.png}
    \caption{Porównanie cen rzeczywistych i przewidywanych przez model bazowy.}
    \label{fig:base_model_predictions}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{assets/base_error_distribution.png}
    \caption{Rozkład błędów predykcji dla modelu bazowego.}
    \label{fig:base_model_error_distribution}
\end{figure}

Krzywe uczenia przedstawione na rysunku \ref{fig:base_model_learning_curve} pokazują spadek wartości funkcji straty (MSE) oraz wzrost funkcji zysku (\(R^2\)) zarówno na zbiorze treningowym, jak i walidacyjnym w miarę postępu treningu.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{assets/base_learning_curves.png}
    \caption{Krzywe uczenia modelu bazowego: wartość funkcji straty (MSE) oraz funkcji zysku (\(R^2\)) na zbiorach treningowym i walidacyjnym.}
    \label{fig:base_model_learning_curve}
\end{figure}

Wyniki modelu bazowego stanowią punkt odniesienia do oceny skuteczności pozostałych dwóch modeli oraz pozwalają na wstępną ocenę, jak dobrze prosta architektura LSTM radzi sobie z zadaniem prognozowania cen akcji na podstawie ich historycznych wartości.
...
Celem utworzenia modelu wzbogaconego było zbadanie hipotezy pobocznej -- Czy dodanie dodatkowego kontekstu rynkowego do oryignalnego zbioru danych poprawi jakość predykcyjną.
Do całości badania, utworzenie tego modelu, wnosi kolejną wartość porównawczą. %! CHECK IT
Osiągnięte wartości metryk przez model wzbogacony przedstawia tabela \ref{tab:enhanced_model_metrics}

% Model wzbogacony, który wykorzystywał dodatkowe wskaźniki analizy technicznej jako cechy wejściowe, osiągnął następujące wyniki na zbiorze testowym:
...
Wskaźnik \(R^2\) na poziomie 0.7961 oznacza, że model wyjaśnia około 79.6\% wariancji w danych testowych, co oznacza spadek zdolności do generalizacji w porównaniu do modelu bazowego.

Wizualna ocena modelu (rys. \ref{fig:enhanced_model_predictions} i \ref{fig:enhanced_model_error_distribution}) potwierdza wyniki metryk. 
Chociaż model nadal podąża za ogólnym trendem, jego prognozy wykazują większe odchylenia od wartości rzeczywistych. 
Histogram błędów jest szerszy i bardziej spłaszczony niż w przypadku modelu bazowego, co świadczy o większej wariancji błędów predykcji.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{assets/enhanced_actual_vs_predicted.png}
    \caption{Porównanie cen rzeczywistych i przewidywanych przez model wzbogacony.}
    \label{fig:enhanced_model_predictions}  
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{assets/enhanced_error_distribution.png}
    \caption{Rozkład błędów predykcji dla modelu wzbogaconego.}
    \label{fig:enhanced_model_error_distribution}
\end{figure}

Krzywe uczenia (rys. \ref{fig:enhanced_model_learning_curve}) również wskazują na trudniejszy proces treningu. Mimo że model nie wykazuje jednoznacznych oznak przeuczenia, finalny błąd na zbiorze walidacyjnym jest wyższy niż w modelu bazowym.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{assets/enhanced_learning_curves.png}
    \caption{Krzywe uczenia modelu wzbogaconego: wartość funkcji straty (MSE) oraz funkcji zysku (\(R^2\)) na zbiorach treningowym i walidacyjnym.}
    \label{fig:enhanced_model_learning_curve}
\end{figure}

Uzyskane wyniki sugerują, że dostarczenie modelowi dodatkowych informacji w postaci wskaźników analizy technicznej nie poprawiło, a wręcz pogorszyło jakość predykcji. 
...
Wizualna ocena wyników na danych filtrowanych (rys. \ref{fig:kalman_filtered_predictions} i \ref{fig:kalman_filtered_error_distribution}) pokazuje bardzo bliskie dopasowanie przewidywanych cen do rzeczywistych wartości, a histogram błędów ma wąski rozkład, co świadczy o niskiej wariancji błędów predykcji i dobrej generalizacji modelu.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{assets/kalman_actual_vs_predicted_filtered.png}
    \caption{Porównanie cen filtrowanych i przewidywanych przez model z filtrem Kalmana.}
    \label{fig:kalman_filtered_predictions}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{assets/kalman_error_distribution_filtered.png}
    \caption{Rozkład błędów predykcji dla modelu z filtrem Kalmana na danych filtrowanych.}
    \label{fig:kalman_filtered_error_distribution}
\end{figure}
...
Rysunki \ref{fig:kalman_original_predictions} oraz \ref{fig:kalman_original_error_distribution} ilustrują porównanie przewidywanych cen z rzeczywistymi wartościami oraz rozkład błędów predykcji na oryginalnych danych.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{assets/kalman_actual_vs_predicted_original.png}
    \caption{Porównanie cen rzeczywistych i przewidywanych przez model z filtrem Kalmana na oryginalnych danych.}
    \label{fig:kalman_original_predictions}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{assets/kalman_error_distribution_original.png}
    \caption{Rozkład błędów predykcji dla modelu z filtrem Kalmana na oryginalnych danych.}
    \label{fig:kalman_original_error_distribution}
\end{figure}

Krzywe uczenia dla modelu z filtrem Kalmana (rys. \ref{fig:kalman_model_learning_curve}) pokazują, że model dobrze uczył się na danych filtrowanych, jednak jego zdolność do generalizacji na oryginalnych danych była ograniczona.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{assets/kalman_learning_curves.png}
    \caption{Krzywe uczenia modelu z filtrem Kalmana: wartość funkcji straty (MSE) oraz funkcji zysku (\(R^2\)) na zbiorach treningowym i walidacyjnym.}
    \label{fig:kalman_model_learning_curve}
\end{figure}

Wyniki te potwierdzają hipotezę, że filtracja danych za pomocą filtru Kalmana znacząco poprawia jakość prognozy, gdy model jest testowany na odszumionych danych.
Jednakże, w rzeczywistości rynkowej, gdzie dane są obarczone szumem, model ten nie wykazuje przewagi nad modelem bazowym.
Sugeruje to, że choć filtracja może nie być korzystna w przypadku predykcji dokładnych cen akcji, to jednak w przypadku przewidywaniu następnych ruchów cenowych lub trendów, gdzie szum może mieć mniejsze znaczenie, takie podejście mogłoby przynieść korzyści.
...
% Zgodnie z oczekiwaniami, wydajność modelu spada, gdy jest on testowany na oryginalnych, zaszumionych danych. 
% Błąd RMSE wzrasta do 4.6423, a \(R^2\) spada do 0.8685. 
% Co istotne, te wartości są bardzo zbliżone do wyników uzyskanych przez model bazowy (RMSE 4.5626, \(R^2\) 0.8730). 
% Sugeruje to, że model wytrenowany na filtrowanym sygnale, postawiony przed zadaniem predykcji zaszumionych danych, zachowuje się podobnie do modelu, który od początku uczył się na tych zaszumionych danych.
