...
% \begin{enumerate}
%     \item \textbf{Inicjalizacja:} Na początku, wagi \( \mathbf{W}^{(l)} \) oraz obciążenia \( \mathbf{b}^{(l)} \) są inicjalizowane losowo.
%     \item \textbf{Propagacja w przód:} Dane wejściowe są przekazywane przez sieć, generując prognozy \( \mathbf{\hat{y}} \).
%     \item \textbf{Obliczanie błędu:} Błąd prognozy jest mierzony za pomocą funkcji kosztu (np. MSE, Cross-Entropy), porównując prognozy \( \mathbf{\hat{y}} \) z rzeczywistymi wartościami \( \mathbf{y} \).
%     \item \textbf{Propagacja wsteczna:} Błąd jest propagowany wstecz przez sieć, obliczając gradienty funkcji kosztu względem wag i obciążeń.
%     \item \textbf{Aktualizacja parametrów:} Wagi i obciążenia są aktualizowane za pomocą algorytmu optymalizacji (np. SGD, Adam), wykorzystując obliczone gradienty.
% \end{enumerate}
% Proces jest powtarzany przez wiele iteracji (epok) na całym zbiorze treningowym, aż do osiągnięcia zadowalającej dokładności prognoz.
