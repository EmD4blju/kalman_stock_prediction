...
\begin{quote}
    ...
    Główny wniosek płynący z pracy jest taki, że filtracja Kalmana, choć nie gwarantuje lepszych prognoz konkretnych wartości cen akcji, to znacznie poprawia zdolność modelu do nauki trendu, co mogłoby się znacznie lepiej sprawdzić w zadaniach natury klasyfikacyjnej, np. prognozowania kierunku zmian cen.

    \vspace{1cm}
    \noindent \textbf{Słowa kluczowe:} prognozowanie cen akcji, sieci LSTM, filtr Kalmana, szeregi czasowe, uczenie maszynowe, analiza finansowa.
\end{quote}
\clearpage
...
\begin{quote}
    ...
    This thesis investigates the effectiveness of the Kalman filter as a data pre-processing technique for denoising input data to improve the prediction quality of an LSTM network. An experiment was conducted on historical stock price data for Amazon.com Inc. (AMZN), comparing three models: a baseline LSTM, a model enriched with additional technical analysis indicators, and a model trained on filtered data.

    The results showed that in a realistic test scenario, the simplest baseline model was the most effective. The model trained on filtered data, despite showing no advantage over the baseline model in the realistic test, achieved high performance (\(R^2=0.9756\)) when tested on denoised data, demonstrating its ability to almost perfectly extract the underlying trend.
    ...
    \vspace{1cm}
    \noindent \textbf{Keywords:} stock price prediction, LSTM networks, Kalman filter, time series, machine learning, financial analysis.
\end{quote}
\clearpage
...
Pierwszym praktycznym zastosowaniem filtru Kalmana było śledzenie trajektorii rakiet w programie kosmicznym Apollo.
Od tamtej pory filtr Kalmana znalazł szerokie zastosowanie w różnych dziedzinach, takich jak robotyka, nawigacja czy ekonomia \cite{welch1995introduction}.
...
W kontekście prognozowania cen akcji, filtr Kalmana może być użyty do odszumienia danych w fazie ich wstępnego przetwarzania.
...
\end{itemize}
Podział danych został przeprowadzony w sposób chronologiczny, aby zachować naturalną kolejność czasową szeregów danych giełdowych i uniknąć potencjalnych problemów związanych z przeciekiem danych \cite{goodfellow-et-al-2016}.
...
Rysunek \ref{fig:training_pipeline} przedstawia schematycznie zwizualizowany proces uczenia.
% \begin{figure}[H]
%     \centering
%     % \includegraphics[width=1\textwidth]{assets/training_pipeline.png}
%     \caption{Schemat zwizualizowanego procesu uczenia modelu.}
...
Takie podejście zapewniło nie tylko przejrzystość i łatwość zarządzania eksperymentami, ale przede wszystkim gwarantowało powtarzalność (ang. \textit{reproducibility}) procesu uczenia dla każdego z trzech modeli.
...
\end{itemize}
Cały proces, od generowania metryk po wizualizacje, został zaimplementowany jako zautomatyzowany pipeline w ramach projektu Kedro, co zapewniło spójność i powtarzalność całego eksperymentu. 
