...
Przedstawiona architektura oraz proces propagacji w przód definiują przepływ informacji w sieci neuronowej w celu generowania prognoz.
Jednakże aby prognoza była trafna, parametry sieci -- wartości wag \( \mathbf{W}^{(l)} \) oraz obciążeń \( \mathbf{b}^{(l)} \) -- muszą zostać odpowiednio dobrane.
...
Innymi słowy, sieć uczy się na podstawie przykładów, aby poprawić swoje prognozy.

Aby proces uczenia zwracał rzetelne wyniki, zestaw danych dzieli się na trzy niezależne podzbiory:
\begin{itemize}
    \item \textbf{Zbiór treningowy:} Służy do uczenia sieci poprzez dostosowywanie jej parametrów.
    \item \textbf{Zbiór walidacyjny:} Używany do monitorowania wydajności sieci podczas treningu i dostrajania hiperparametrów.
    \item \textbf{Zbiór testowy:} Służy do oceny ostatecznej wydajności sieci na niewidzianych wcześniej danych.
\end{itemize}
...
\begin{enumerate}
    ...
    Formalnie gradient jest pochodną funkcji kosztu względem wag lub obciążeń:
    \begin{equation}
...
Wynikowo sieć neuronowa uczy się optymalnych wartości wag i obciążeń, co pozwala jej na generowanie trafnych prognoz na niewidzianych wcześniej danych.
...
Sieci rekurencyjne (RNN) zostały zaprojektowane właśnie w tym celu.

Główną różnicą sieci rekurencyjnych w porównaniu do tradycyjnych sieci jest inna architektura neuronów.
W sieciach RNN neurony te nazywane są komórkami rekurencyjnymi (RNN cells).
Posiadają one dodatkowe połączenie zwrotne (ang. feedback connection), które pozwala na przekazywanie informacji z poprzednich kroków czasowych do bieżącego kroku.
Formalnie, w sieciach RNN, stan ukryty \( \mathbf{h}_t \) w czasie \( t \) jest obliczany na podstawie bieżącego wejścia \( \mathbf{x}_t \) oraz poprzedniego stanu ukrytego \( \mathbf{h}_{t-1} \):
\begin{equation}
...
W celu wygenerowania prognozy \( \mathbf{\hat{y}}_t \) w czasie \( t \), sieć wykorzystuje stan ukryty:
\begin{equation}
...
\begin{figure}[H]
    ...
    \caption{Budowa komórki rekurencyjnej w sieci RNN. Opracowanie własne na podstawie \href{https://commons.wikimedia.org/wiki/File:Recurrent_neural_network_unfold.svg}{Wikimedia Commons}.}
    \label{fig:rnn_cell}
\end{figure}

Taka architektura stanowi solidny fundament do modelowania szeregów czasowych.
Niemniej jednak, tradycyjne sieci RNN mają pewne ograniczenia.
Są nimi m.in. problem zanikającego i eksplodującego gradientu podczas procesu uczenia, co utrudnia naukę długoterminowych zależności w danych.
...
Ze względu na współdzielenie wag w czasie, gradienty są sumowane przez wszystkie kroki czasowe, co daje ostateczny gradient dla każdej wagi.
...
\begin{itemize}
    ...
    W konsekwencji sieć nie jest w stanie nauczyć się zależności między danymi, które są od siebie odległe w czasie.
    ...
    Na skutek tego aktualizacje wag stają się tak duże, że proces uczenia staje się niestabilny.
\end{itemize}
